...
const getArticle = async (res, req, offset, limit) => {
  if (offset && limit) {
    const allArticles = await Articles.findAll({ offset,
      limit,
      attributes: ['id', 'slug', 'title', 'description', 'body', 'tagList', 'createdAt', 'updatedAt'],
      include: [
        { model: User,
          as: 'author',
  ...
  // eslint-disable-next-line no-else-return
    ...
    const allArticles1 = await Articles.findAll({ attributes: ['id', 'slug', 'title', 'description', 'body', 'tagList', 'createdAt', 'updatedAt'],
      include: [
        { model: User,
          as: 'author',
          attributes: ['username', 'bio', 'image', 'following'] }
